...
// Next.js 파일 컨벤션: /tw 및 하위 세그먼트 전체가 이 이미지를 og:image 로 자동 채택.
// output:"export" 에서 빌드 타임에 PNG로 정적 생성. 크기는 KR/EN/JA og-image와 동일(878×443).
...
async function loadTcFont(text: string, weight: number): Promise<ArrayBuffer> {
  const url = `https://fonts.googleapis.com/css?family=Noto+Sans+TC:${weight}&text=${encodeURIComponent(text)}`;
...
export default async function Image() {
  const glyphs = "雖然慢，但方向是對的一步往前走";
  const [fontData, mascotBuf] = await Promise.all([
    loadTcFont(glyphs, 900),
  ...
  return new ImageResponse(
      ...
      >
        ...
        >
          <div style={{ display: "flex" }}>
            <span>雖然慢，</span>
            <span style={{ color: "#6bade8" }}>但方向是對的</span>
          </div>
          <div style={{ display: "flex" }}>
            <span style={{ color: "#f0a050" }}>一步一步</span>
            <span>往前走</span>
          </div>
        </div>
        <img
    ...
    {
      ...size,
      fonts: [{ name: "NotoTC", data: fontData, style: "normal", weight: 900 }],
    }
  );
}
